...
function App() {
  ...
  const setTemp = (t: number) => setInput((prev) => ({ ...prev, temp: t }))
  const setMinutes = (m: number) => setInput((prev) => ({ ...prev, minutes: m }))

  useEffect(() => {
    saveLastInput({ temp, unit, minutes })
  }, [temp, unit, minutes])
  ...
  const handleUnitChange = (next: Unit) => {
    ...
    setInput((prev) => ({ ...prev, unit: next, temp: snapped }))
  }
